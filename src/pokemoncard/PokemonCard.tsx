...
export function pokemonCard({ image, name, id, types }: PokemonCardProps) {
    return (
        <div className="rounded-[20px] overflow-hidden max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 max-w-[275px] h-full max-h-[350px] w-full m-[auto] hover:scale-110 transform transition duration-500">
            <Link to={`/detail/${name}`} >
                <img className='rounded-t-lg max-h-[218px] h-full p-[40px] w-full' src={image} />
            </Link>
            <div className='pb-5'>
                <div className='flex justify-between'>
                    <h5 className="capitalize mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{name.toUpperCase()}</h5>
                    {
                        (id) < 9 ?
                            <h5 className='mb-2 text-xl font-bold tracking-tight text-gray-900 dark:text-white'>#00{id}</h5> :
                            (id) > 9 && (id) < 99 ?
                                <h5 className='mb-2 text-xl font-bold tracking-tight text-gray-900 dark:text-white'>#0{id}</h5> :
