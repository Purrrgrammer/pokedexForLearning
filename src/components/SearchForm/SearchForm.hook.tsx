...
const useSearchForm = () => {
  ...
  const { setFetchPokemonList, setPokemonList, fetchPokemon } =
    usePokemonListStore();
  ...
  // function for filtering pokemon
  const filterPokemon = (
    ...
    const filteringKeyword = pokeList.filter((item) =>
      item.name.toLowerCase().includes(keyword?.toLowerCase())
    );
    // ERROR can be undefinded so the to
    console.log("filteringKeyword", filteringKeyword);
    const filteringType =
      type !== "all types"
        ? filteringKeyword.filter((item) =>
